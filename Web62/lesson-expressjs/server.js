...
app.put("/contacts/:id", (req, res) => {
  ...
  const isExit = contacts.findIndex((item) => item.id === idParam)
  if (isExit < 0) {
    res.status(404).json({message: 'Not found'})
  } else {
    const newContact = { ...contacts[isExit], ...req.body }
    contacts.splice(isExit, 1, newContact)
    res.json({
    msg: "Add new contact successfully",
    data: contacts,
  });
